...
for DIST_KEY in DIST_KEYS:
    ...
    print(f"\nAnalisi Distribuzione: {dist_name}")
    ...
    # Gestione caso limite se min_err == max_err (es. 0)
    if y_box_margin == 0: y_box_margin = 1e-3

    common_box_ylim = (max(0, min_err - y_box_margin), max_err + y_box_margin)


    # Funzione helper per disegnare boxplot personalizzati con logica avanzata
    def draw_custom_boxplot(ax, data, color, title, label_y=True):
        ...
        # 2. Disegno Boxplot
        # Medianprops color='k' per distinguerla visivamente all'interno del box
        ax.boxplot(data, medianprops=dict(color='black', linewidth=1.5))

        # 3. Disegno Linee Orizzontali
        ...
        # 5. Logica Ordinamento (Chi sta sopra?)
        ...
        # 6. Calcolo Sovrapposizione basato sui limiti comuni calcolati fuori
        y_range = common_box_ylim[1] - common_box_ylim[0]
        if y_range == 0: y_range = 1.0

        dist = abs(mean_val - median_val)
        overlap_threshold = 0.07 * y_range  # 7% dell'altezza totale

        text_x_offset = 1.02  # Margine destro fisso
        ...
        ax.set_xticks([])  # Rimuoviamo i tick x
    ...
    # Aumentiamo leggermente il padding per far stare le etichette laterali
